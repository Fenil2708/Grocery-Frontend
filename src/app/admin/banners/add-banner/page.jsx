...
const AddBanner = () => {
  ...
  const handlePublish = (e) => {
    ...
    postData("/api/banner/create", postFields).then((res) => {
      setIsLoading(false);
      if (res.success) {
        context?.alertBox("success", "Banner Published Successfully");
      ...
      setIsLoading(false);
      console.error(err);
      context?.alertBox("error", "Failed to publish banner");
    })
  }
    ...
    <section className="w-full py-3 px-5">
      ...
      <form className="mt-5 bg-white p-5 shadow-md rounded-md" onSubmit={handlePublish}>
        <div className="flex flex-col gap-4">
          ...
          >
            {isLoading ? <CircularProgress size={24} color="inherit" /> : "Publish Banner"}
          </Button>
        </div>
      </form>
    </section>
