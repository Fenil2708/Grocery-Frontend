...
import { deleteImages, fetchDataFromApi, putData } from "@/admin-utils/api";
...
const EditSlide = () => {
  ...
  const handleUpdate = (e) => {
    ...
    putData(`/api/homeSlider/${id}`, postFields).then((res) => {
      setIsLoading(false);
      if (res.success) {
      ...
      setIsLoading(false);
      console.error(err);
      context?.alertBox("error", "Failed to update slide");
    })
  }
    ...
    <section className="w-full py-3 px-5">
      ...
      
      {
        isFetching ? (
            ...
            <form className="mt-5 bg-white p-5 shadow-md rounded-md" onSubmit={handleUpdate}>
                <div className="flex flex-col gap-4">
                  ...
                  >
                      {isLoading ? <CircularProgress size={24} color="inherit" /> : "Update Slide"}
                  </Button>
                </div>
            </form>
        )
